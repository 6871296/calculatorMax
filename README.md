...
页面操作什么的大家都懂，这里指只讲解一些专有名词。

### simpleeval模式

simpleeval模式是另一种计算算式的方法。它基于`simpleeval`库中的`simple_eval`函数，比`eval`函数更安全，但功能更少（但少的只有“高级”、“执行类”和未列出的Python代码。该模式~~尽管是开发的一个累赘但我还是~~会持续更新，加入新的功能。
